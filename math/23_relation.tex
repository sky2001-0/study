...
\thm{標準分解}{
  ...
  \thmref{両立２}より、写像$h \colon X / \sim_{f} \rightarrow Y$が存在して、$\Im(h) = \Im(f)$。
...
\dfn{集合におけるフィルター}{
  集合$X$について、以下を満たす集合$\mathcal{F} \in 2^{2^X}$を集合$X$のフィルターと呼ぶ。
  \eqg*{
    \varnothing \notin \mathcal{F} \land X \in \mathcal{F} \\*
    \forall F_1, F_2 \in \mathcal{F} \qty(F_1 \cap F_2 \in \mathcal{F}) \\*
    \forall F \in F \forall G \in 2^X \qty(F \subset G \rightarrow G \in \mathcal{F})
  }
}

\cor*{
  集合$X$のフィルターは、\lemref{包含は順序}より与えられる半順序集合$\qty(2^X, \subset)$のフィルターである。
}
...
\thm{集合の超フィルター}{
  ...
  $\mathcal{S} \coloneqq \qty{S \in 2^X \mid A \cup S \in \mathcal{F}}$を考える。定義より$\mathcal{F} \subset \mathcal{S} \land X \setminus A \in \mathcal{S}$である。
...
\dfn{普遍}{
  集合$X$上のネット$\qty(a_\lambda)_{\lambda \in \Lambda}$が以下を満たすとき、$\qty(a_\lambda)_{\lambda \in \Lambda}$は普遍であると呼ぶ。
  \eq*{
...
\thm{ネットの定めるフィルター}{
  ...
  $F_1, F_2 \in \mathcal{F}$について、定義より$\exists \lambda_1, \lambda_2 \subset \Lambda \qty(\qty(x_\lambda)_{\lambda \in \Lambda_{\succcurlyeq \lambda_1}} \in F_1 \land \qty(x_\lambda)_{\lambda \in \Lambda_{\succcurlyeq \lambda_2}} \subset F_2)$である。
  ...
  $\forall F \in \mathcal{F} \forall G \in 2^X$について、$F \subset G$ならば定義より明らかに$G \in \mathcal{F}$
}
