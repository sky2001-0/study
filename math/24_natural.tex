...
\dfn{後者関数}{
  無限系譜$X$について、以下のように定める写像$s \colon X \rightarrow X$を後者関数（successor）と呼ぶ。
  \eq*{
...
\lem*{
  \eq*{
    \forall X \qty(M(X) \rightarrow \exists Y \qty(M(Y) \land Y = \bigcap \qty{Z \in 2^X \mid M(Z)}))
  }
}{
  \axiref{冪集合の公理}、および\thmref{分出の公理図式}より、
  \eq*{
    \exists Y \qty(Y = \bigcap \qty{Z \in 2^X \mid M(Z)})
  }
  ここで$X \in \qty{Z \in 2^X \mid M(Z)}$より、$\qty{Z \in 2^X \mid M(Z)} \neq \varnothing$
...
\lem*{
  \eq*{
    \forall X \forall Y \qty(M(X) \land M(Y) \rightarrow \bigcap \qty{Z \in 2^X \mid M(Z)} = \bigcap \qty{Z \in 2^Y \mid M(Z)})
  }
}{
  $\omega(X) \coloneqq \bigcap \qty{Z \in 2^X \mid M(Z)}$とする。

  \mlemref{-2}より、$M(X \cap Y)$で、$\omega(X) \subset \omega(X \cap Y)$

  今、
  \eqg*{
    \forall Z \qty(Z \in \qty{Z \in 2^X \mid M(Z)}) \\*
    \forall Z \qty(M(Z) \land Z \subset X) \\*
    \forall Z \qty(M(Z \cap Y) \land Z \cap Y \subset X \cap Y) \\*
    \forall Z \qty(Z \cap Y \in \qty{Z \in 2^{X \cap Y} \mid M(Z)})
  }
  ゆえに、
  \eqg*{
    \forall z \forall Z \qty(z \in \omega(X \cap Y) \land Z \in \qty{Z \in 2^X \mid M(Z)}) \\*
...
\dfn{自然数}{
  \mlemref{0}より、無限系譜$X$の取り方によらず$\bigcap \qty{Z \in 2^X \mid M(Z)}$が一意に定まり、これを自然数$\N$（または$\omega$）と呼ぶ。誤解のない範囲で、自然数$\N$の要素も自然数と呼ぶ。
}
...
\thmf{\textit{Peano}の公理}{Peanoの公理}{
  ...
  定理より、$\N = \bigcap \qty{Z \in 2^E \mid M(Z)}$

  $\forall n \qty(n \in \N \rightarrow n \in \bigcap \qty{Z \in 2^E \mid M(Z)})$
...
\dfn{前者関数}{
  後者関数$s$は、\thmref{Peanoの公理}の第四式より単射であるため、左逆写像を持つ。これを前者関数$p \colon \N \setminus \qty{0} \rightarrow \N$と呼ぶ。
}
...
\dfn{自然数の順序}{
  \lemref{包含は順序}より、自然数$\N$上で半順序$\leq$を以下のように定める。
  \eq*{
...
\lem*{
  以下で定める部分写像$+ \colon \N \times \N \nrightarrow \N$は左全域的である。
  \eqg*{
...
\dfn{自然数の加法}{
  \mlemref{0}より定まる$\N$上の二項演算を加法と呼び、$+$で表す。
...
\lem*{
  以下で定める部分写像$\times \colon \N \times \N \nrightarrow \N$は左全域的である。
  \eqg*{
...
\dfn{自然数の除法}{
  \mlemref{-1}、\mlemref{0}より、以下の写像$\divisionsymbol \colon \N \times \N \setminus \qty{0} \rightarrow \N \times \N$を定義できる。
  \eq*{
