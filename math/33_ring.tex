...
\dfn{環準同型}{
  環$R, R'$について以下を満たす写像$\varphi \colon R \rightarrow R'$が存在するとき、$\varphi$を環準同型写像、または単に環準同型と呼ぶ。
  \begin{itemize}
    \item 群$\qty(R, +), \qty(R', +)$について、群準同型。
    \item モノイド$\qty(R, \times), \qty(R', \times)$について、モノイド準同型。
  \end{itemize}
}
...
\thm{極大左イデアルの存在}{
  ...
  $T = \qty{J \in 2^R \mid J \text{は左イデアル} \land I \subset J \land J \neq R}$を考える。
...
\thm{環準同型定理}{
  環$R, R'$と、環準同型$f \colon R \rightarrow R'$、$f$に付随する同値関係$\sim_{f}$について、環同型$\bar{f} \colon R / \sim_{f} \rightarrow \Im(f)$が存在する。
}{
  $\Im(f)$は$R'$の部分環である。
...
$R / \sim_f = R / \Ker (f)$であるため、$R / \Ker (f) \rightarrow \Im (f)$の形で書かれることが多い。
