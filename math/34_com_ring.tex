...
\dfn{素元分解}{
  ...
  \begin{itemize}
    \item $\exists n \in \N \exists p \in \Map(n, \mathcal{P}(D)) \exists u \in D^\times \qty(a = u \prod_{m = 0}^n p(m))$
  \end{itemize}
}
...
\thm{UFDの可逆でない既約元は素元}{
  ...
  $n = 2$とすると、$a u^{-1}$は既約であることより、$p(0)$または$p(1)$が可逆元であるが、\thmref{整域の素元は可逆でない既約元}に反する。
...
\thmf{PIDは\textit{Noether}}{PIDはNoether}{
  単項イデアル整域$D$について、$2^D$上の以下を満たす点列$\qty(I_n)_{n \in \N}$を考える。
  \begin{itemize}
...
\thm{PIDはUFD}{
  ...
  これは\thmref{整域の素元は可逆でない既約元}に反する。ゆえに$\aqty{a} \subsetneq \aqty{a_1}$
  ...
  $n' \neq 0$のとき、\thmref{整域の素元は可逆でない既約元}より$\prod_{m = 0}^{n'} p'(m)$は可逆でない。
  ...
  以下の$q \in \Map(n', \mathcal{P}(D))$を考える。
  \eq*{
    q(m) =
    \begin{cases}
      p(m) & \qty(m \neq k) \\*
...
\dfnf{\textit{Euclid}整域}{Euclid整域}{
  整域$D$について、以下を満たす写像$f \colon D \setminus \qty{0_D} \rightarrow \N$が存在するとき、$D$を\textit{Euclid}整域と呼ぶ。
  \eqg*{
...
\thm{体準同型は単射}{
  体$F$、零環ではない環$R$について、環準同型$\varphi \colon F \rightarrow R$は単射である。
}{
...
\dfn{多項式環}{
  ...
  \eq*{
    R[X] \coloneqq \qty{\varphi \in \Map(\N, R) \mid \exists N \in \N \forall n \in \N_{\geq N} \qty(\varphi(n) = 0_R)}
  }
...
\lem*{
  以下で定義する写像$\gamma \colon R \rightarrow R[X]$を考える。
  \eq*{
    \gamma(a)(n) \coloneqq
    \begin{cases}
      a & \qty(n = 0) \\*
...
\dfn{多項式の値}{
  可換環$R$上の多項式環$R[X]$について、以下で定める写像$f \colon R[X] \rightarrow \Map(R, R)$が存在する。
  \eq*{
