...
\lem{整数の準備：前順序}{
  直積集合$\N \times \N$上で以下の自己関係$\preccurlyeq_Z$は前順序である。
  \eq*{
    \forall \qty(a_1, a_2), \qty(b_1, b_2) \in \N \times \N \qty(\qty(a_1, a_2) \preccurlyeq_Z \qty(b_1, b_2) \defiff a_1 + b_2 \leq b_1 + a_2)
  }
}{
  反射的である。\\*
...
\lem{整数の準備：前順序の全域性}{
  直積集合$\N \times \N$上で\lemref{整数の準備：前順序}の前順序$\preccurlyeq_Z$は以下を満たす。
  \eq*{
    \forall a, b \in \N \times \N \qty(a \preccurlyeq_Z b \lor b \preccurlyeq_Z a)
  }
}{
  \thmref{自然数の全順序}より明らか。
}
...
\lem{整数の準備：加法可換群}{
  直積集合$\N \times \N$上の以下の演算$+$は可換群をなす。
  \eq*{
    \forall \qty(a_1, a_2), \qty(b_1, b_2) \in \N \times \N \qty(\qty(a_1, a_2) + \qty(b_1, b_2) \defiff \qty(a_1 + b_1, a_2 + b_2))
  }
}{
...
\lem{整数の準備：可換環}{
  \lemref{整数の準備：加法可換群}の定める可換群$\qty(\N \times \N, +)$上の以下の演算$\times$は可換環をなす。
  \eq*{
    \forall \qty(a_1, a_2), \qty(b_1, b_2) \in \N \times \N \qty(\qty(a_1, a_2) \times \qty(b_1, b_2) \defiff \qty(a_1 b_1 + a_2 b_2, a_1 b_2 + a_2 b_1))
  }
}{
  結合法則を示す。
  \eqa*{
...
\lem{整数の準備：同値類}{
  直積集合$\N \times \N$上で以下の自己関係$\sim_Z$は同値類である。
  \eq*{
    \forall \qty(a_1, a_2), \qty(b_1, b_2) \in \N \times \N \qty(\qty(a_1, a_2) \sim_Z \qty(b_1, b_2) \defiff \qty(a_1, a_2) \preccurlyeq \qty(b_1, b_2) \land \qty(b_1, b_2) \preccurlyeq \qty(a_1, a_2))
  }
}{
...
\lem{整数の準備：両立}{
  直積集合$\N \times \N$上で\lemref{整数の準備：同値類}の同値類$\sim_Z$は、\lemref{整数の準備：前順序}の前順序、\lemref{整数の準備：加法可換群}の加法、\lemref{整数の準備：可換環}の乗法のそれぞれと両立する。
}{
...
\dfn{整数}{
  \lemref{整数の準備：両立}より定まる商マグマ$\qty(\N \times \N) / \sim_Z$を整数と呼び、$\Z$で表す。
  また、その元も整数と呼ぶ。
...
\lem*{
  写像$\varphi \colon \N \rightarrow \Z, \varphi(n) = \qty[\qty(n, 0)]$は以下を満たす。
  \begin{enumerate}
    \item 単射
    \item 加法についてモノイド準同型
    \item 乗法についてモノイド準同型
...
\thmf{整数は\textit{Euclid}整域}{整数はEuclid整域}{
  ...
  以下で定める写像$\sign \colon \Z \rightarrow \qty{1, -1}$を考える。
  \eq*{
    \sign(x) =
    \begin{cases}
      1 & \qty(x \geq 0) \\*
  ...
  写像$\abs{} \colon \Z \ni x \rightarrow x \sign(x) \in \Z$を考える。
