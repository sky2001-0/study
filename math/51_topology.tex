...
\dfn{開基}{
  空でない集合$X$について、以下を満たす集合系$\mathcal{B} \in 2^{2^X}$を開基と呼ぶ。
  \eqg*{
...
\dfn{開集合系}{
  空でない集合$X$について、以下を満たす集合系$\mathcal{O} \in 2^{2^X}$を開集合系、または位相と呼ぶ。
  \eqg*{
    X \in \mathcal{O} \\*
    \forall O_1, O_2 \in \mathcal{O} \qty(O_1 \cap O_2 \in \mathcal{O}) \\*
    \forall A \in 2^\mathcal{O} \qty(\bigcup A \in \mathcal{O})
  }
...
\cor*{
  空でない集合$X$の開集合系$\mathcal{O}$について、
  \eq*{
    \varnothing \in \mathcal{O}
  }
}

\thm{開基の定める位相}{
  空でない集合$X$の開基$\mathcal{B}$について、以下で与える集合系$\mathcal{O}$は開集合系である。
  \eq*{
    \mathcal{O} \coloneqq \qty{O \in 2^X \mid \forall x \in O \exists B \in \mathcal{B} \qty(x \in B \land B \subset O)}
  }
}{
  第一式を示す。\dfnref{開基}第一式より、$\forall x \in X \exists B \in \mathcal{B} \qty(x \in B \land B \subset X)$であるので、$X \in \mathcal{O}$\\*
  ...
  \dfnref{開基}第二式より、$B_1 \cap B_2 \in \mathcal{B}$であるので、$O_1 \cap O_2 \in \mathcal{O}$\\*

  第三式を示す。$A \in 2^\mathcal{O}$について、$\forall x \in \bigcup A \exists O \in \mathcal{O} \exists B \in \mathcal{B} \qty(x \in B \land B \subset O \land O \subset A)$であるので、$\bigcup A \in \mathcal{O}$
}

\lem{開集合系の一意性}{
  集合$X$の開基$\mathcal{B}$と集合系$\mathcal{B'} \in 2^{2^X}$について、以下を満たすとき、$\mathcal{B'}$は開基である。
  \eqg*{
    \forall B' \in \mathcal{B'} \forall x \in B' \exists B \in \mathcal{B} \qty(x \in B \land B \subset B') \\*
    \forall B \in \mathcal{B} \forall x \in B \exists B' \in \mathcal{B'} \qty(x \in B' \land B' \subset B)
  }

  さらに\thmref{開基の定める位相}の定める開集合系$\mathcal{O}$と$\mathcal{O'}$は一致する。
}{
...
\lem{開集合系は開基}{
  空でない集合$X$の開集合系$\mathcal{O}$について、$\mathcal{O}$は開基である。

  さらに開集合系$\mathcal{O}$から\thmref{開基の定める位相}の定める開集合系$\mathcal{O'}$は$\mathcal{O}$に一致する。
}{
  \dfnref{開集合系}第一式、第二式より、開基である。\\*

  \thmref{開基の定める位相}より$\forall O' \in \mathcal{O'} \forall x \in O' \exists O \in \mathcal{O} \qty(x \in O \land O \subset O')$である。

  また、\thmref{開基の定める位相}より$\mathcal{O} \subset \mathcal{O'}$を得る。
...
\lem*{
  位相空間$\qty(X, \mathcal{O})$と、$X$の空でない部分集合$A$を考える。$\mathcal{O}$を与える開基$\mathcal{B}$について、$\qty{A \cap B \mid B \in \mathcal{B}}$は集合$A$の開基である。
}{
  定義より明らか。
}

\dfn{部分空間}{
  位相空間$\qty(X, \mathcal{O})$と、$X$の空でない部分集合$A$を考える。\mlemref{0}の与える開基の与える開集合系$\mathcal{O'}$について、位相空間$\qty(A, \mathcal{O'})$を部分空間と呼ぶ。
}

\lem*{
  位相空間$\qty(X, \mathcal{O}), \qty(X', \mathcal{O'})$と$\mathcal{O}, \mathcal{O'}$を与える開基$\mathcal{B}, \mathcal{B'}$について、$\mathcal{B} \times \mathcal{B'}$は$X \times X'$上の開基である。
}{
  \dfnref{開基}第一式より$\forall \qty(x, x') \in X \times X' \exists B \in \mathcal{B} \exists B' \in \mathcal{B'} \qty(x \in B \land x' \in B')$である。ゆえに、$\qty(x, x') \in B \times B'$\\*

  $\forall \qty(B_1, B'_1), \qty(B_2, B'_2) \in \mathcal{B} \times \mathcal{B'}$を考える。

  $\forall \qty(x, x') \in \qty(B_1 \cap B_2, B'_1 \cap B'_2)$について、\dfnref{開基}第二式より$\exists B \in \mathcal{B} \exists B' \in \mathcal{B'} \qty(x \in B \subset B_1 \cap B_2 \land x' \in B' \subset B'_1 \cap B'_2)$

  \corref{共通と直積}より、$\qty(B_1 \cap B_2, B'_1 \cap B'_2) = \qty(B_1, B_2) \cap \qty(B'_1, B'_2)$であるので成り立つ。
}

\dfn{直積空間}{
  位相空間$\qty(X, \mathcal{O}), \qty(X', \mathcal{O'})$について、\mlemref{0}と\thmref{開基の定める位相}より定まる位相空間$\qty(X \times X', \mathcal{O''})$を直積空間と呼ぶ。また位相$\mathcal{O''}$を箱位相と呼ぶ。
}

\dfn{連続}{
  位相空間$\qty(X, \mathcal{O}), \qty(X', \mathcal{O'})$について、写像$f \colon X \rightarrow X'$が以下を満たすとき、$f$は連続であると呼ぶ。
  \eq*{
...
\lem{開基と連続}{
  位相空間$\qty(X, \mathcal{O}), \qty(X', \mathcal{O'})$と$X'$の開基$\mathcal{B'}$、および写像$f \colon X \rightarrow X'$を考える。
  このとき、以下の２つは同値である。
  \begin{enumerate}
    \item $f$は連続。
  ...
  \dfnref{開集合系}より$\forall x \in O' \exists B' \in \mathcal{B'} \qty(x \in B' \land B' \subset O')$である。

  ゆえに、$O' = \bigcup \qty{B'(x) \mid x \in O'}$である。。

  \thmref{原像の性質}より$f^{-1}(O') = \bigcup \qty{f^{-1}(B'(x)) \mid x \in O'}$であるので、\dfnref{開集合系}第三式より示される。
}

\thm{連続写像の合成}{
  位相空間$X, Y, Z$について、写像$f \colon X \rightarrow Y, g \colon Y \rightarrow Z$がともに連続であるとき、合成$g \circ f$は連続である。
}{
  $\forall O_Z \in \mathcal{O}_Z \qty(g^{-1}(O_Z) \in \mathcal{O}_Y)$であり、$f^{-1}(g^{-1}(O_Z)) \in \mathcal{O}_X$である。
}

\dfn{同相}{
  位相空間$X, X'$と写像$f \colon X\rightarrow X'$について、$f$が全単射かつ$f$と$f^{-1}$がともに連続であるとき、$f$を同相写像と呼ぶ。

  また、同相写像$f \colon X \rightarrow Y$が存在するとき、$X$と$Y$は位相同型または同相であると呼ぶ。
}
...
\dfn{基本近傍系}{
  空でない集合$X$とその任意の点$x$について、以下を満たす集合系$\mathcal{B}(x) \in 2^{2^X}$を基本近傍系と呼ぶ。
  \eqg*{
    \mathcal{B}(x) \neq \varnothing \\*
...
\thm{開基の定める基本近傍系}{
  ...
  その任意の点$x$について、以下を満たす集合系$\mathcal{B}(x)$は基本近傍系である。
  \eq*{
...
\thm{基本近傍系の定める位相}{
  ...
  このとき、以下の集合$\mathcal{O}$は$X$の開集合系である。
  \eq*{
    \mathcal{O} \coloneqq \qty{O \in 2^X \mid \forall x \in O \exists B \in \mathcal{B}(x) \qty(B \subset O)}
  }

  さらに開基$\mathcal{B}$について、\thmref{開基の定める位相}の定める開集合系$\mathcal{O}_1$と、\thmref{開基の定める基本近傍系}とこの定理により定まる開集合系$\mathcal{O}_2$は、一致する。
}{
  ...
  以下より一致する。
  \eq*{
    \mathcal{O}_2 = \qty{O \in 2^X \mid \forall x \in O \exists B \in \mathcal{B}(x) \qty(B \subset O)} = \qty{O \in 2^X \mid \forall x \in O \exists B \in \mathcal{B} \qty(x \in B \land B \subset O)} = \mathcal{O}_1
  }
}
...
\dfn{近傍系}{
  ...
  \eq*{
    \mathcal{N}(x) \coloneqq \qty{N \in 2^X \mid \exists B \in \mathcal{B}(x) \qty(B \subset N)}
  }
}

\lem{近傍系の一意性}{
  空でない集合$X$の基本近傍系$\mathcal{B}(x)$と集合系$\mathcal{B'}(x) \in 2^{2^X}$について、以下を満たすとき、$\mathcal{B'}(x)$は基本近傍系である。
  \eqg*{
    \forall B' \in \mathcal{B'}(x) \exists B \in \mathcal{B}(x) \qty(B \subset B') \\*
    \forall B \in \mathcal{B}(x) \exists B' \in \mathcal{B'}(x) \qty(B' \subset B)
  }

  さらに\dfnref{近傍系}の定める近傍系$\mathcal{N}(x)$と$\mathcal{N'}(x)$は一致する。
}{
...
\lem{近傍系は基本近傍系}{
  ...
  $\forall B \in \mathcal{B}(x)$について、$B \in \mathcal{N}(x)$である。

  \lemref{近傍系の一意性}より成り立つ。
}

\lem{基本近傍系の定める位相の定める近傍系}{
  空でない集合$X$と基本近傍系$\mathcal{B}(x)$について、\thmref{基本近傍系の定める位相}と\thmref{開基の定める基本近傍系}の定める基本近傍系$\mathcal{B'}(x)$を考える。

  $\mathcal{B}(x), \mathcal{B'}(x)$から\dfnref{近傍系}より定まる近傍系$\mathcal{N}(x), \mathcal{N'}(x)$について、$\mathcal{N}(x) = \mathcal{N'}(x)$である。
}{
  定義より、以下である。
  \eq*{
    \mathcal{B'}(x) = \qty{B' \in 2^X \mid x \in B' \land \forall y \in B' \exists D \in \mathcal{B}(y) \qty(D \subset B')}
  }
...
\dfn{点連続}{
  位相空間$X, X'$と点$x \in X$、写像$f \colon X \rightarrow X'$について、以下を満たすとき、写像$f$は点$x$で連続であると言う。
  \eq*{
...
\cor*{
  位相空間$X, X', X''$と点$x \in X$、写像$f \colon X \rightarrow X', g \colon X' \rightarrow X''$について、$f$は点$x$で連続であり、$g$が点$f(x)$で連続であるとき、合成写像$g \circ f$は点$x$で連続である。
}

\thm{点連続と収束}{
  位相空間$X, X'$と点$x \in X$、写像$f \colon X \rightarrow X'$と、近傍系$\mathcal{N}(x)$を与える基本近傍系$\mathcal{B}(x)$について、以下の３つは同値である。
  \begin{enumerate}
    \item $f$が$x$で連続
...
\thm{連続と点連続}{
  ...
  今、$x \in f^{-1}(B')$であるので、\thmref{開基の定める位相}より$\exists B \in \mathcal{B}(x) \qty(B \subset f^{-1}(B'))$となる。
...
\dfn{閉集合系}{
  ...
  \eq*{
    \mathcal{F} \coloneqq \qty{F \in 2^X \mid X \setminus F \in \mathcal{O}}
  }
...
\lem{コンパクトの言い換え}{
  ...
  \eq*{
    \forall A \in 2^\mathcal{F} \qty(A \neq \varnothing \land \bigcap A = \varnothing \rightarrow \exists A' \in 2^A \qty(A' \neq \varnothing \land \bigcap A' = \varnothing \land \abs{A'} < \infty))
  }
}{
...
\thm{ネットによるコンパクトの特徴づけ}{
  ...
  $P \coloneqq \qty{A \in C \mid \abs{A} < \infty \land A \neq \varnothing}$について$\qty(P, \subset)$は有向集合であり、
...
\thm{コンパクト空間の連続像はコンパクト}{
  コンパクトな位相空間$\qty(X, \mathcal{O})$と、位相空間$\qty(X', \mathcal{O'})$について、連続写像$f \colon X \rightarrow X'$を考える。
  ...
  コンパクト性から、$\exists A' \in 2^{C'} \qty(\abs{A'} < \infty \land X = \bigcup \qty{f^{-1}(O') \mid O' \in A'})$
...
\thmf{\textit{Hausdorff}性}{Hausdorff性}{
  ...
  \thmref{選択公理が与える写像}より、写像$g \colon \mathcal{N}(x) \times \mathcal{N}(y) \rightarrow X, g \qty(\qty(N_x, N_y)) \in N_x \cap N_y$が存在する。
...
\thm*{
  ...
  すなわち、写像$\varphi_1 \colon \N \rightarrow \mathcal{O}$が存在して、$F_1 \subset \bigcup \qty{\varphi_1(n) \mid n \in \N} \land \forall n \in \N \qty(\bar{\varphi_1(n)} \cap F_2 = \varnothing)$\\*

  同様に、写像$\varphi_2 \colon \N \rightarrow \mathcal{O}$が存在して、$F_2 \subset \bigcup \qty{\varphi_2(n) \mid n \in \N} \land \forall n \in \N \qty(\bar{\varphi_2(n)} \cap F_1 = \varnothing)$\\*
...
\thm{非連結}{
  ...
  \begin{enumerate}
    ...
    \item $\exists A, B \in 2^X \setminus \qty{\varnothing} \qty(X = A \cup B \land A \cap \bar{B} = \varnothing \land \bar{A} \cap B = \varnothing)$
  \end{enumerate}
}{
  $1. \rightarrow 2.$を考える。
...
\thm{連結空間の連続像は連結}{
  連結な位相空間$\qty(X, \mathcal{O})$と、位相空間$\qty(X', \mathcal{O'})$について、連続写像$f \colon X \rightarrow X'$を考える。
  このとき、像$f(X)$は連結である。
}{
  $f(X)$が連結でないと仮定する。
...
\thm{第一可算空間における点連続と収束}{
  第一可算な位相空間$X$、位相空間$X'$、点$x \in X$、写像$f \colon X \rightarrow X'$について、以下の２つは同値である。
  \begin{enumerate}
    \item $f$が$x$で連続
  ...
  \eq*{
    \exists N' \in \mathcal{N}(f(x)) \forall n \in \N \exists y \in B_n(x) \qty(y \notin N')
  }
  \thmref{選択公理が与える写像}より、写像$g \colon \N \ni n \rightarrow B_n(x) \in X, f(B_n(x)) = y \notin N'$が存在する。
...
\thm{第二可算空間の満たす性質}{
  ...
  開被覆$C$について、\thmref{開基の定める位相}より$\forall x \in X \exists O \in C \exists n \in \N \qty(x \in B_n \land B_n \subset O)$
...
\thmf{\textit{Bolzano-Weierstrass}の定理}{Bolzano-Weierstrassの定理}{
  ...
  $X$上の点列$\qty(x_k)_{k \in \N}$について、$\qty(F_k)_{k \in \N}, F_k = \qty{x_l \mid l \in \N \land l \geq k} \in 2^X$を考える。\\*
  ...
  ここで、以下の写像$\varphi : \N \rightarrow \N$を考える。
  \eqg*{
...
\thmf{\textit{Bolzano-Weierstrass}の定理の逆}{Bolzano-Weierstrassの定理の逆}{
  第二可算で点列コンパクトな位相空間$X$は、コンパクトである。
}{
  コンパクトでないと仮定する。その任意の有限部分集合が開被覆とならない$X$の開被覆$C$が存在する。

  \thmref{第二可算空間の満たす性質}より可算な部分開被覆$C' \coloneqq \qty{O_n \mid n \in \N}$が存在する。
...
\lem{順序の開基}{
  ２点以上を含む全順序集合$\qty(X, \leq)$について、以下の集合系$\mathcal{B}$は$X$の開基である。
  \eqg*{
    \mathcal{D} \coloneqq \qty{\qty{x \in X \mid a < x} \mid a \in X} \cup \qty{\qty{x \in X \mid x < b} \mid b \in X} \\*
    \mathcal{B} \coloneqq \qty{\bigcap A \mid A \in 2^\mathcal{D} \land \abs{A} < \infty}
  }
}{
  第一式を示す。２点以上を含むため$\exists c \in X \qty(c < x \lor x < c)$である。ゆえに$\exists D \in \mathcal{D} \qty(x \in D)$である。

  $x \in D = \bigcap \qty{D} \in \mathcal{B}$より成り立つ。\\*

  第二式を示す。$\forall A_1, A_2 \in 2^{\mathcal{D}} \qty(\abs{A_1} < \infty \land \abs{A_2} \rightarrow \abs{A_1 \cup A_2} < \infty)$より、$\bigcap \qty(A_1 \cup A_2)$は求める集合である。
}

\dfn{順序位相}{
  \lemref{順序の開基}と\thmref{開基の定める位相}より定まる位相空間$\qty(X, \mathcal{O})$を順序空間と呼ぶ。
}

\lem{開・閉区間は、開・閉集合}{
  順序空間$\qty(X, \mathcal{O})$について、開区間は開集合であり、閉区間は閉集合である。
}{
  開区間$\sqty{a, b} = \bigcap \qty{\qty{x \in X \mid a < x}, \qty{x \in X \mid x < b}} \in \mathcal{B} \subset \mathcal{O}$である。\\*

  閉区間$\qty[a, b]$を考える。$X \setminus \qty[a, b] = \bigcup \qty{\qty{x \in X \mid x < a}, \qty{x \in X \mid b < x}}$である。

	$\qty{x \in X \mid x < a}, \qty{x \in X \mid b < x} \in \mathcal{D} \subset \mathcal{B} \subset \mathcal{O}$より成り立つ。
}