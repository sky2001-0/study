...
\thm{開集合系の公理}{
  ...
  第三式を示す。$A \in 2^\mathcal{O}$について、$\forall x \in \bigcup A \exists O \in \mathcal{O} \exists B \in \mathcal{B} \qty(x \in B \land B \subset O \land O \subset \bigcup A)$であるので$\bigcup A \in \mathcal{O}$
}
...
\lem*{
  空でない集合$X$と、位相空間$\qty(X', \mathcal{O'})$、$\mathcal{O'}$を与える$X$の開基$\mathcal{B'}$、写像$f \colon X \rightarrow X'$を考える。
  ...
  第三式を考える。\thmref{原像の性質}と\thmref{開集合系の公理}第三式より$\forall A' \in 2^{\mathcal{O'}} \qty(\bigcup  \qty{f^{-1}(O') \mid O' \in A'} = f^{-1}(\bigcup A') \in \mathcal{O}_2)$
...
\dfn{誘導空間}{
  空でない集合$X$と、位相空間$\qty(X', \mathcal{O'})$、写像$f \colon X \rightarrow X'$について、\mlemref{0}より定まる開集合系$\mathcal{O}$が存在する。
...
\lem*{
  ...
  \corref{共通と直積}より、$\qty(B_{11} \cap B_{12}, B_{21} \cap B_{22}) = \qty(B_{11}, B_{21}) \cap \qty(B_{12}, B_{22})$であるので成り立つ。\\*
...
\lem{直積と連続}{
  位相空間$\qty(X_1, \mathcal{O}_1), \qty(X_2, \mathcal{O}_2), \qty(Y, \mathcal{O})$について、連続写像$f \colon X_1 \times X_2 \rightarrow Y$を考える。

  このとき、$\forall w \in X_1$について写像$f_w \colon X_2 \ni x \mapsto f(w, x) \in Y$は連続である。
}{
...
\lem*{
  ...
  以下より一致する。
  \eq*{
    \mathcal{O'} = \qty{O \in 2^X \mid \forall x \in O \exists B \in \mathcal{B}(x) \qty(B \subset O)} = \qty{O \in 2^X \mid \forall x \in O \exists B \in \mathcal{B} \qty(x \in B \land B \subset O)} = \mathcal{O}
  }\\*
...
\cor*{
  位相空間$X$について、以下が成り立つ。
  \eqg*{
    \forall A, B \in 2^X \qty(A \subset B \rightarrow \bar{A} \subset \bar{B}) \\*
    \forall D \in 2^{2^X} \qty(\bigcup \qty{\bar{A} \mid A \in D} \subset \bar{\bigcup D})
  }
}
...
\lem{順序の開基}{
  ...
  $\forall B \in \mathcal{B} \qty(-\infty \notin B \land \infty \notin B)$であるので、$\mathcal{B} \in 2^{2^X}$である。\\*
