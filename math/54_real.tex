...
\lem*{
  写像$\varphi \colon \Q \rightarrow \R, \varphi(a) = \qty[\qty(a)_{n \in \N}]$は以下を満たす。
  \begin{enumerate}
    \item 単射
    \item 加法、乗法について環準同型
    \item 全順序と両立
...
\thm{実数の位相と一様構造}{
  ...
  $\mathcal{B} \coloneqq \qty{\sqty{a, b} \mid a, b \in \R}$は開基である。

  $\mathcal{B'} \coloneqq \qty{\sqty{a', b'} \mid a', b' \in \Q}$を考える。
  $\mathcal{B'}$は可算である。

  定義より$\mathcal{B'} \subset \mathcal{B}$である。

  \thmref{実数の有理数による近似}より$\forall a, b \in \R \forall x \in \R \qty(a < x < b \rightarrow \exists a', b' \in \Q \qty(a < a' < x < b' < b))$である。

  \lemref{開集合系の一意性}より$\mathcal{B'}$は開基である。
}

\lem*{
  全単射$\varphi \colon \qty[0, 1] \rightarrow \R$が存在する。
}{
  以下の写像は、全単射$f \colon \sqty{0, 1} \rightarrow \R$である。
  \eq*{
    f(x) \coloneqq \frac{1 / 2 - x}{x \qty(x - 1)}
  }

  以下の写像は、全単射$g \colon \qty[0, 1] \rightarrow \sqty{0, 1}$である。
  \eq*{
    g(x) \coloneqq
    \begin{cases}
...
\thm{連続体濃度}{
  全単射$2^\N \rightarrow \R$が存在する。
}{
  $f \colon \R \rightarrow 2^\Q, f(r) = \qty{q \in \Q \mid q < r}$は\thmref{実数の有理数による近似}より単射である。\\*

  以下で定める写像$g \colon 2^\N \rightarrow \qty[0, 1]$を考える。

  $A \in 2^\N$について以下の点列$\qty(y_m)_{m \in \N}$が与えられる。
  \eq*{
    y_m \coloneqq
    \begin{cases}
      1 & \qty(m \in A) \\*
  ...
  \mlemref{-1}より単射$\varphi$が存在して、単射$\varphi \circ g \colon 2^\N \rightarrow \R$が存在する。\\*

  \thmref{Bernsteinの定理}より成り立つ。
}

\thm{実数の直積は連続体濃度}{
  全単射$\R \rightarrow \R \times \R$が存在する。
}{
  \thmref{連続体濃度}と\thmref{自然数の直積は可算}より成り立つ。
}
...
\lem*{
  ...
  以下のような写像$N_0 \colon \N \rightarrow \N$を考える。
  \eqg*{
    N_0(0) \coloneqq N(0) \\*
...
\thm{実数の完備性}{
  ...
  \lemref{R上のCauchy列は収束する}から、\thmref{可算一様空間における完備}より成り立つ。
}
...
\thm{最大値の定理}{
  空でないコンパクト空間$X$上で定義された連続関数$f \colon X \rightarrow \R$は最大値を持つ。
}{
...
\thm{中間値の定理}{
  連結空間$X$上で定義された連続関数$f \colon X \rightarrow \R$について、以下が成り立つ。
  \eq*{
